  ...
  public CheckoutView(
    ...
    user.ifPresent(
        u -> {
          ...
          Anchor exportExcelAnchor =
              new Anchor(
                  new StreamResource(
                      LocalDate.now().toString().concat(" Aanwezigheidslijst.xlsx"),
                      Exporter.exportAsExcel(attendeesGrid)),
                  "Export to Excel");

          final var sendMailButton =
              new Button(
                  "Send Mail",
                  onClick -> {
                    try {
          ...
          changesLayout.add(attendeesGrid, exportExcelAnchor, sendMailButton);
