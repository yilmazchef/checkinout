...
public class CheckinView extends VerticalLayout {
  ...
  public CheckinView(
    ...
    user.ifPresent(
        organizer -> {
          final var attendeesGrid = new Grid<ChecksGridData>();
          attendeesGrid
          ...
          attendeesGrid.setItems(checkRepository.findAllCheckinsOfToday());
  ...
  private void checkInUser(
      final User organizer, final Grid<ChecksGridData> attendeesGrid, final String scannedQRCode) {
    ...
    if (oAttendee.isPresent()) {
      ...
      if (hasCheckedInBefore.isEmpty()) {
        ...
        if (foundUser.isPresent()) attendeesGrid.setItems(checkRepository.findAllCheckinsOfToday());
        ...
        Notification.show(
                ("Already Checked Before (?): " + scannedQRCode),
                4000,
                Notification.Position.BOTTOM_CENTER)
            .open();
      }
      ...
      Notification.show(
              ("Invalid or Unregistered QR ! ->  " + scannedQRCode),
              4000,
              Notification.Position.BOTTOM_CENTER)
          .open();
    }
